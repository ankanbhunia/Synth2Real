...
def get_file_paths(path):
    return sorted([os.path.join(root, file)  for root, dirs, files in os.walk(path) for file in files])
...
class BatchGenerator():
    ...
    def __init__(self,
        ...
        self.image_paths = [] # The list of images from which the generator will draw.
         # The dictionary of ground truth images that correspond to the images.
        self.num_classes = num_classes
        self.dataset_size = 0
        self.ground_truth = False # Whether or not ground truth images were given.
        
        self.image_paths = get_file_paths(self.image_dirs)
        self.image_paths = [i for i in self.image_paths if image_file_extension in i]
        
        self.ground_truth_paths = {}
        for i in self.image_paths:
            img_name = i.split('/')[-1]
            self.ground_truth_paths[i] = os.path.join(self.ground_truth_dirs,img_name)
        ...
        while True:
            ...
            for image_path in self.image_paths[current:current+batch_size]: # Careful: This works in Python, but might cause an 'index out of bounds' error in other languages if `current+batch_size > len(image_paths)`
                ...
                if self.ground_truth:
                    ...
                    gt_image_path = self.ground_truth_paths[image_path]
