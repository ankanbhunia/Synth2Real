...
if opts.trainer == 'MUNIT':
    ...
    outputs = (outputs + 1) / 2.
    path = os.path.join(opts.output_folder, 'output'+opts.input.split('/')[-1])
    vutils.save_image(outputs.data, path, padding=0, normalize=True)
else:
    pass
...
if not opts.output_only:
    ...
    vutils.save_image(image.data, os.path.join(opts.output_folder, 'input'+opts.input.split('/')[-1]), padding=0, normalize=True)
